...
import { useFollowing, useFollowMutation } from "@/hooks/useFollow";
...
export default function FollowingModal({
  ...
  const { data, isLoading } = useFollowing(userId, 1, 50);
  ...
  const following = (data?.data?.following || []).filter(
    (user): user is NonNullable<typeof user> => user != null
  );

  // Get user IDs to fetch follow status
  const userIds = useMemo(() => following.map((user) => user._id), [following]);
  ...
  // Check follow status: first local state, then from API
  // For "following" list, default to true since these are users being followed
  const isUserFollowing = (targetUserId: string) => {
    if (followingState[targetUserId] !== undefined) {
      return followingState[targetUserId];
    }
    // If we have data from API, use it; otherwise default to true (since this is "following" list)
    if (followStatusMap[targetUserId] !== undefined) {
      return followStatusMap[targetUserId];
    }
    return true;
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
      <div className="bg-[#262626] rounded-xl w-full max-w-md max-h-[400px] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h3 className="text-white font-semibold">Đang theo dõi</h3>
